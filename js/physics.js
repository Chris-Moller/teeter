...
export function updatePhysics(dt, tiltAngle, pitch) {
  ...
  return updateOnTrack(dt, tiltAngle, pitch || 0);
}
...
function updateOnTrack(dt, tiltAngle, pitch) {
  ...
  // Forward motion modulated by pitch (forward tilt = faster, backward = slower)
  ball.vz = Math.max(0, Math.min(MAX_SPEED, FORWARD_SPEED * (1 + pitch * PITCH_SENSITIVITY)));
